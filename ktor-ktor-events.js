(function (factory) {
  ...
  function Events() {
    this.d3a_1 = new CopyOnWriteHashMap();
  }
  protoOf(Events).e3a = function (definition, value) {
    var exception = null;
    var tmp0_safe_receiver = this.d3a_1.g2c(definition);
    if (tmp0_safe_receiver == null)
      null;
    else {
      // Inline function 'io.ktor.util.internal.LockFreeLinkedListHead.forEach' call
      var tmp = tmp0_safe_receiver.w1p();
      var cur = tmp instanceof LockFreeLinkedListNode ? tmp : THROW_CCE();
      while (!equals(cur, tmp0_safe_receiver)) {
        if (cur instanceof HandlerRegistration) {
          var registration = cur;
          try {
            var tmp_0 = registration.g3a_1;
